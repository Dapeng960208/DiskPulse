...
TELEMETRY_TERMINAL_CONSTRAINT = "ck_telemetry_run_terminal_fields"
OLD_TELEMETRY_TERMINAL_CONDITION = (
...
NEW_TELEMETRY_TERMINAL_CONDITION = (
    "(outcome IS NULL AND finished_at IS NULL AND data_state IS NULL AND "
    "records_written IS NULL AND error_code IS NULL) OR "
    "(outcome IS NOT NULL AND finished_at IS NOT NULL AND ((outcome = 'success' "
    "AND data_state IS NOT NULL AND records_written IS NOT NULL AND error_code IS NULL) OR "
...
def _sqlite_telemetry_table(*, terminal_condition: str) -> sa.Table:
    """Return the revision-009 telemetry shape for SQLite batch DDL."""
    ...
    table = sa.Table(
        ...
        sa.CheckConstraint(terminal_condition, name=TELEMETRY_TERMINAL_CONSTRAINT),
        sa.ForeignKeyConstraint(
            ["storage_cluster_id"], ["storage_clusters.id"], ondelete="SET NULL"
        ),
...
def _replace_telemetry_terminal_constraint(
    condition: str, *, copy_from: sa.Table | None = None
) -> None:
    with op.batch_alter_table(
        ...
        batch_op.drop_constraint(TELEMETRY_TERMINAL_CONSTRAINT, type_="check")
        batch_op.create_check_constraint(TELEMETRY_TERMINAL_CONSTRAINT, condition)


def _upgrade_telemetry_terminal_constraint() -> None:
    if op.get_context().dialect.name == "sqlite":
        _replace_telemetry_terminal_constraint(
            NEW_TELEMETRY_TERMINAL_CONDITION,
            copy_from=_sqlite_telemetry_table(
                terminal_condition=OLD_TELEMETRY_TERMINAL_CONDITION
            ),
        )
        return
    _replace_telemetry_terminal_constraint(NEW_TELEMETRY_TERMINAL_CONDITION)


def _downgrade_telemetry_terminal_constraint() -> None:
    if op.get_context().dialect.name == "sqlite":
        _replace_telemetry_terminal_constraint(
            OLD_TELEMETRY_TERMINAL_CONDITION,
            copy_from=_sqlite_telemetry_table(
                terminal_condition=NEW_TELEMETRY_TERMINAL_CONDITION
            ),
        )
        return
    _replace_telemetry_terminal_constraint(OLD_TELEMETRY_TERMINAL_CONDITION)


def upgrade() -> None:
    _upgrade_telemetry_terminal_constraint()
    op.create_table(
...
def downgrade() -> None:
    for table in (
        ...
        op.drop_table(table)
    _downgrade_telemetry_terminal_constraint()
