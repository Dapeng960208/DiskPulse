...
def _alert(*, alert_id: int, external_event_id: str, severity: str, event_code: str):
    return models.StorageAlerts(
        id=alert_id,
        storage_cluster_id=7,
        source="netapp",
        external_event_id=external_event_id,
        fingerprint=f"netapp:{event_code}:node:node-{alert_id}",
        severity=severity,
...
def _evidence(*, incident_id: int, external_event_id: str, evidence_hash: str):
    return models.IncidentEvidence(
        incident_id=incident_id,
        source="storage_alert",
        source_ref=f"netapp:{external_event_id}",
...
def test_incident_and_ai_diagnosis_expose_chinese_gap_and_safe_vendor_evidence_summaries(
    ...
    assert "存储集群或节点" in gap["description"]
...
def test_legacy_noncritical_vendor_fault_reconciliation_is_dry_run_safe_and_idempotent(
    ...
    preview = service.reconcile_legacy_vendor_incidents(db_session, dry_run=True, now=NOW)
