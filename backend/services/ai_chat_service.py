...
_AUDIT_TOOL_NAMES = frozenset({"list_audit_events", "get_audit_event"})
_AUDIT_ANALYSIS_PROMPT = """审计研判约束：仅可使用当前会话实际提供、且已由接口权限校验允许的只读工具。
先明确要查询的项目或时间范围；需要查找事件时使用 list_audit_events，需要下钻单条事件时使用 get_audit_event。可用时应先列表再详情，避免把列表摘要当作完整事实。
按 operation_id、trace_id 或 request_id 配对同一操作的尝试与结果；展示层关联仅用于当前范围研判，不可改写原始审计历史。脱敏字段、空结果、无权限和未提供的数据都是数据缺口，不得推断为事实。
可结合其他已经授权的相关只读工具交叉核验，但不得调用写操作、承诺执行处置或编造工具结果。
审计研判最终回答必须按以下四个二级标题组织：
## 研判依据
## 排查建议
## 解决方案
## 限制与数据缺口"""
