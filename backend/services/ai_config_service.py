...
def create_model(db: Session, payload: AIModelCreate, actor_id: int) -> dict:
...
def update_model(db: Session, model_id: int, payload: AIModelPatch, actor_id: int) -> dict:
...
def test_model(db: Session, model_id: int) -> dict:
    model = _get_or_404(db, model_id)
    try:
