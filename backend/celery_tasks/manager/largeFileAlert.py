...
from models import StorageAlerts, User, LargeFiles, Group, Host
...
class LargeFileState:
    ...
    def check_large_file_status(self) -> Set[int]:
        """
        Check status of large files across all groups.
        Returns set of file IDs that should be deleted.
        """
        files_to_delete: Set[int] = set()
        total_files_checked = 0
        groups_without_host = 0
        groups_ssh_failed = 0

        try:
            group_dbs = self._get_all_groups()
            total_groups = len(group_dbs)
            self.logger.info(f"Starting large file check for {total_groups} groups")

            for group_db in group_dbs:
                # Get monitor host for the group

                host_db = self.db.query(Host).filter_by(id=group_db.monitor_host_id).first()
                if not host_db:
                    groups_without_host += 1
                    self.logger.warning(
                        f"Group {group_db.linux_path} skipped: monitor host not found "
                        f"(host_id={group_db.monitor_host_id})"
                    )
                    continue

                # Get large files for the group
                large_files_db = self.db.query(LargeFiles).filter_by(group_id=group_db.id).all()
                if not large_files_db:
                    self.logger.warning(f"Group {group_db.linux_path} has no large files")
                    continue

                # Create SSH client
                try:
                    ssh_client = SSHClientBase(
                        hostname=host_db.ip,
                        port=22,
                    ...
                    groups_ssh_failed += 1
                    self.logger.warning(
                        f"Group {group_db.linux_path} skipped: SSH connection failed "
                        f"(host={host_db.ip}, error={str(e)})"
                    )
                    continue

                # Check each large file
                group_files_checked = 0
                group_files_to_delete = 0

                for large_file_db in large_files_db:
                    total_files_checked += 1
                    group_files_checked += 1

                    success, output = ssh_client.execute_command(f'stat "{large_file_db.linux_path}"',
                                                                 check_output=False)
                    if not success:
                        files_to_delete.add(large_file_db.id)
                        group_files_to_delete += 1
                        self.logger.debug(
                            f"File marked for deletion: ID={large_file_db.id}, "
                            f"path={large_file_db.linux_path}"
                        )

                self.logger.info(
                    f"Group {group_db.linux_path}: checked {group_files_checked} files, "
                    f"{group_files_to_delete} marked for deletion"
                )
                ...
                try:
                    ...
                    self.logger.warning(
                        f"SSH connection close error for group {group_db.id}: {str(e)}"
                    )

            # Log overall summary
            self.logger.info(
                f"Large file check completed: {total_groups} groups processed, "
                f"{total_files_checked} files checked, "
                f"{len(files_to_delete)} files to delete, "
                f"{groups_without_host} groups without host, "
                f"{groups_ssh_failed} groups with SSH failure"
            )

            return files_to_delete

        except Exception as e:
            self.logger.error(f"Large file check failed: {str(e)}")
            raise

    def _delete_large_files(self, file_ids: Set[int]) -> int:
        """
        Delete large file records from database.
        ...
        try:
            ...
            self.logger.info(f"Starting deletion of {len(file_ids_list)} file records")

            deleted_count = self.db.query(LargeFiles).filter(LargeFiles.id.in_(file_ids_list)).delete(
                synchronize_session=False
            )
            if self.environment and self.environment != 'dev':
                self.db.commit()

            if deleted_count != len(file_ids_list):
                self.logger.warning(
                    f"Deletion count mismatch: expected {len(file_ids_list)}, "
                    f"deleted {deleted_count}"
                )

            self.logger.info(f"Deleted {deleted_count} large file records")
            return deleted_count

        except Exception as e:
            self.logger.error(f"Failed to delete large files: {str(e)}")
            self.db.rollback()

    def execute_full_check_and_cleanup(self) -> int:
        """
        Execute complete workflow: check file status and clean up database.
        Returns number of deleted records.
        """
        try:
            self.logger.info("Starting full large file check and cleanup workflow")

            files_to_delete = self.check_large_file_status()

            if not files_to_delete:
                self.logger.info("No files need to be deleted")
                return 0

            deleted_count = self._delete_large_files(files_to_delete)
            self.logger.info(f"Cleanup completed: {deleted_count} files deleted")
            return deleted_count

        except Exception as e:
            self.logger.error(f"Cleanup workflow failed: {str(e)}")
