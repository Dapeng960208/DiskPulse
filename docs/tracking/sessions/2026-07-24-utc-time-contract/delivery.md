...
- 整改自查补齐：025 迁移在任何 DDL 前拒绝非空开发库，并为 PostgreSQL 设置锁/语句超时；Dashboard 默认查询范围保持 aware UTC；存储健康导出按当前用户时区格式化，原始字典 API 响应统一为 UTC `Z`。
...
- 整改自查后端聚焦：`test_utc_time_contract.py`、`test_datetime_utils.py`、`test_storage_health_analytics.py`、`test_dashboard_overview.py`，133 passed；QuestDB 时间契约门禁 `test_questdb_time_contract_guard.py test_datetime_utils.py`，19 passed。
...
- 025 现在会拒绝所有非空数据库；这符合本开发模式的空库重建约束，但执行迁移前仍需停止写入者，并确认目标确为开发库。
