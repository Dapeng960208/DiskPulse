...
- 本次迁移升降级：临时 SQLite 库最小旧表 + `stamp 000000000019` 后执行 `upgrade 000000000020` 与 `downgrade 000000000019`，通过。
...
- 完整 Alembic 从空库升到 head 的 SQLite 验证被既有 `000000000019` PostgreSQL interval SQL 阻断；本次 `000000000020` 已通过独立升降级验证。
