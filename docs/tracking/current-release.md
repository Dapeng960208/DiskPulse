...
- 前端在两个资源详情中按服务端能力懒加载“容量预测”页签，展示趋势、区间、质量、样本量、新鲜度、计划和审计摘要；超级管理员可在 `/admin/forecast-governance` 发布给项目成员、创建候选并启用满足评估门槛的候选。
- 审查修复覆盖页签懒加载、白名单质量摘要、非有限分位数、超时回退、成功预测审计、回退汇总、候选版本展示、模型停用复核、重复窗口/版本、预测新鲜度、无效容量计划、404 空态和重复 HTTP 审计。
