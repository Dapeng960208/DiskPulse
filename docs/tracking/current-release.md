...
## 2026-07-16：写入表单体验重构设计

- 已完成并确认全站写入表单重构设计，覆盖资源新增/编辑、AI 模型配置、系统设置以及删除、备份移动/回滚、LDAP 同步等高风险确认入口。
- 已选方案为“共享样式 + 增强现有 `useForm` / `useDialog`”：简单表单使用紧凑单列，复杂表单使用宽弹窗、语义分组和双列布局，视觉采用品牌强化方向。
- 交互规则已确认：失焦校验、提交首错定位、重复提交保护、实际修改后的未保存关闭提醒、普通保存轻提示和高风险操作二次确认。
- 本阶段仅完成设计，前端实现、TDD 用例、全量回归、lint、生产构建和登录态浏览器验收均待后续实施；字段语义、payload、API、权限和后端流程不在变更范围内。
- 设计与验收边界见 [写入表单体验重构设计](../features/form-experience/design.md)。
