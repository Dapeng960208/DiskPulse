...
- 合并最新 `main` 前，后端全量测试 `599 passed`；容量预测、统一审计和迁移聚焦测试 `92 passed`；前端容量预测与路由合同聚焦测试通过，scoped ESLint 和 `npx vite build` 成功。
- 容量预测迁移在合并主线卷性能迁移后顺延为 `000000000013`；最终全量前端 coverage、Alembic 唯一 head 和合并后回归结果在本次合入完成前复核。
