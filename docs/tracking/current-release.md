...
- 前端在两个资源详情中按服务端能力显示独立“容量预测”按钮，用户点击后进入懒加载的预测视图，展示趋势、区间、质量、样本量、新鲜度、计划和审计摘要；用户目录、项目组和容量池详情不再显示重复的实时监控标题与副标题；超级管理员可在 `/admin/forecast-governance` 发布给项目成员、创建候选并启用满足评估门槛的候选。
