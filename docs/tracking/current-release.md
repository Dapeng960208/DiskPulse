...
- 已统一存储集群、用户、项目、项目组、项目组标签、用户目录、AI 模型和系统设置的写入体验：弹窗采用品牌渐变标题、顶部标签、语义分组、响应式单/双列网格和具体动作按钮，系统设置采用页面内分组与固定保存区。
...
- 本地浏览器在 `1280×720` 验证紧凑表单为 `480px`，品牌标题和浅色操作区生效；`414×896` 下弹窗宽 `382.4px`，页面与弹窗横向溢出均为 `0`。修改字段后关闭会出现“继续编辑 / 放弃修改”确认。
