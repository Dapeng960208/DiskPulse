...
- 补齐设计审查缺口：页签首次切换才加载预测面板；AI 输入增加白名单质量摘要；非有限分位数、超时和无效输出均回退基线并保留原因；成功预测写入统一审计；治理页展示不含资源身份的 AI 回退汇总；资源页展示当前候选版本而不是误用基线版本。
- 完成代码审查修复：停用的 AI 模型不能再激活候选；三个启用窗口必须互不重复并由数据库唯一约束兜底；重复候选版本返回稳定 `409`；基线按 `training_end` 选择且只叠加同日起点候选；零变化/空白容量计划被拒绝；预测 `404` 显示空态；容量预测领域审计不再被通用中间件重复记录；新增路由不再直接访问容量预测 CRUD。

### 验证状态

- 后端治理、预测中心、统一审计、中间件与迁移链聚焦测试 `92 passed`；新增迁移在 SQLite、PostgreSQL、MySQL 离线编译通过；排除 `backend/.venv` 后源码 `compileall` 通过；前端预测合同和状态测试 `7 passed`、scoped ESLint 和 `npx vite build` 成功。
- `npm run build` 不存在是既有脚本约定问题，已记录在错误库；本轮直接 Vite 生产构建仍保留既有 `%VITE_APP_TITLE%` 和大 chunk 警告。
