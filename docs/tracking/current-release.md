...
- 全站移除页面、详情、页签、弹窗和登录页中标题后直接跟随的描述段落；项目成员与项目审计页签不再重复展示说明段落，项目成员的“添加成员”操作收敛至表格表头。
- `docs/standards/frontend-design-standard.md` 现明确禁止标题后设置描述性副标题，并新增静态 Vitest 门禁防止该结构重新出现。
- “添加成员”仅在项目详情返回 `manage_members` 能力时显示；该能力由后端同时授予项目管理员和超级管理员，后端接口仍保持最终授权校验。

### 验证状态

- RED：全站静态检查识别 15 个标题副标题结构，规范中尚无禁令。
- GREEN：分别执行标题副标题、项目成员、写入表单和 AI 页面聚焦 Vitest，相关用例均通过（36/36）；`cd frontend && npm run build:test` 通过。
- 浏览器：登录页在本地开发服务正常渲染，主题切换后仍无标题副标题结构；已认证的项目成员与项目审计页签未能浏览器复验，因为本地普通服务与 Mock 服务的演示登录请求均未成功完成。

## 2026-07-18：事件详情处置反馈与操作说明

### 已完成

- 事件详情的认领、释放、状态推进、取消静默和维护窗口在请求期间显示操作状态，成功或失败时给出明确反馈；处置按钮、类别、状态和诊断字段补充了与真实服务端语义一致的 Tooltip。
- 新增事件中心操作指南，包含四类事件的触发条件、性能连续异常/遥测盲区标准、证据与置信度、状态机、处置权限和厂商告警白名单的未实现边界；容量压力明确以资源有效规则为准（项目组 → 项目 → 系统，默认硬限额 `80%/90%/95%`），并说明当前没有未处理 SLA 自动升级。

### 验证状态

- RED：详情抽屉缺少认领成功反馈和操作 Tooltip 的聚焦 Vitest 用例按预期失败。
- GREEN：`pnpm exec vitest run test/unit/IncidentDetailDrawer.test.js --coverage.enabled=false` 通过（`3 passed`）；改动 Vue/测试文件 ESLint 通过，`pnpm build:test` 通过。
- 浏览器：主分支 Mock 服务在 `http://localhost:5173/incidents` 和 `/admin/incidents` 均呈白屏且未提供可交互 DOM，控制台没有错误；因此未将浏览器交互验证标记为通过。

## 2026-07-18：事件中心编辑与完整详情 Mock

### 已完成

- 事件中心列表增加“编辑”操作；弹窗可修改 `warning` / `critical` 严重度，并仅提供符合既有状态机的当前状态或下一阶段。
- `/v1/incidents/{id}` 更新接口以白名单校验严重度，记录严重度调整时间线并写入统一审计。
- Mock 事件详情补充资产、诊断候选、两条证据摘要、时间线和按当前账号计算的操作能力，详情抽屉可完整演示。

### 验证状态

- 前端聚焦 Vitest `3 files / 18 tests passed`，受影响 Vue / Mock 文件 ESLint 通过。
- 后端聚焦 pytest `28 passed`，相关 Python 源码编译通过。

## 2026-07-18：资源详情容量预测与 AI 治理

### 已完成

- 新增项目组和用户目录资源预测接口、全局用户可见性、结构化容量计划、候选模型治理、候选回退系统审计及对应数据库迁移。
- 后台使用 45 天训练 + 三个 30 天日级留出窗口持续评估未启用候选；只持久化跨资源聚合 MAPE 和风险覆盖结果，模型输入不含路径、用户标识或跨项目明细。
- 前端在两个资源详情中按服务端能力显示独立“容量预测”按钮，用户点击后进入懒加载的预测视图，展示趋势、区间、质量、样本量、新鲜度、计划和审计摘要；用户目录、项目组和容量池详情不再显示重复的实时监控标题与副标题；超级管理员可在 `/admin/forecast-governance` 发布给项目成员、从 AI 中心全部已配置模型中创建候选并启用满足评估门槛的候选。
- 治理页不显示标题和副标题；全局用户可见性与新增候选模型位于同一控制行。候选模型创建弹窗使用下拉选择，前后端均不按提供方或启用状态限制所选模型；Mock 网关预置候选与完成的评估窗口，便于本地演示。
- Mock 模式为用户目录和项目组预置容量预测访问权限、预测曲线、容量计划及关联事件，资源详情可直接进入容量预测演示。
- 用户目录和项目组的容量预测入口移至列表行“更多操作”，点击进入独立预测详情页；实时监控详情不再承载预测内容。存储空间性能监控页从筛选栏开始展示，移除重复标题与说明。
- 审查修复覆盖页签懒加载、白名单质量摘要、非有限分位数、超时回退、成功预测审计、回退汇总、候选版本展示、配置模型存在性复核、重复窗口/版本、预测新鲜度、无效容量计划、404 空态和重复 HTTP 审计。

### 验证状态

- 合并最新 `main` 后，后端全量测试 `603 passed`，源码 `compileall` 通过；Alembic 保持唯一 `000000000013 (head)`，容量预测迁移已顺延到主线卷性能迁移之后。
- 前端全量 coverage 为 `77 files / 471 passed`：Statements/Lines `97.27%`、Branches `85.88%`、Functions `80.21%`，四项均通过全局门禁；改动文件 ESLint 为 `0 errors`，生产构建成功。
- 审查补充回归覆盖预测 API 映射、全局可见性、候选创建/启用、容量计划成功/失败、事件筛选/分页/详情，以及合并主线后卷监控空响应、组件 Mock 与列表合同兼容性。
- 本轮新增详情入口聚焦回归，验证预测按钮只在服务端允许时显示、可切换至预测视图，以及用户目录、项目组、容量池均隐藏重复标题区。
- 容量预测治理页与 Mock 网关聚焦 Vitest `2 files / 17 tests passed`，覆盖紧凑控制行、所有 AI 中心模型可选以及候选/评估演示数据；后端容量预测治理聚焦 pytest `24 passed`，覆盖公有模型与已停用模型的候选创建和启用。
- 容量预测 Mock 网关聚焦 Vitest `2 files / 13 tests passed`，并通过改动文件 ESLint 与 `npm run build:test`。
- 列表预测导航、详情入口收敛、存储空间性能页筛选顶部布局聚焦 Vitest `5 files / 47 tests passed`；浏览器确认用户目录“更多操作 → 容量预测”进入独立预测页，存储空间页不再显示重复标题区。

### 风险

- 未连接真实 Ollama、PostgreSQL、QuestDB、Redis 或 Celery 执行实际 AI 推理与滚动回测；部署前需先在隔离环境完成端点、数据出口和任务链路验收。
- 构建仍保留既有 `%VITE_APP_TITLE%` 未定义和大体积 chunk 警告；本轮未扩大到构建拆包或品牌配置治理。

## 2026-07-18：存储空间性能采集自动关联

### 已完成

- PostgreSQL `Volume` 新增 `performance_object_id`，容量采集自动保存 NetApp Volume UUID 和 PowerScale Directory Quota ID（无 ID 时使用路径）。
- 性能采集按当前集群稳定身份映射卷对象；未关联的 NetApp 指标、PowerScale workload、父目录或节点指标不会写成卷性能。
- 存储集群性能分析和存储空间性能监控统一按 `performance_object_id` 读取；无法关联时性能序列为空，不使用名称兜底。
- 存储空间详情替换为容量趋势加独立性能图的专用工作区，并保留项目目录与所属集群信息。

### 验证状态

- RED：采集解析器不接受 `volume_identities`，NetApp/Isilon 两条新增用例均按预期失败；稳定身份缺失时旧实现仍会名称回退。
- GREEN：后端卷监控、健康分析、资源映射、软配额、迁移和 schema 契约聚焦回归 `147 passed`；前端聚焦 `2 files / 3 tests passed`；Alembic 唯一 head 为 `000000000012`；`compileall` 通过。
- 前端聚焦 ESLint 与 `build:test` 通过；迁移链/模型契约 `23 passed`，Alembic 唯一 head 为 `000000000012`；浏览器渲染验证因本会话缺少浏览器插件规定的运行入口而未执行。

### 风险与阻塞

- 真实 NetApp/PowerScale/QuestDB 未在本地联调；设备字段、权限与历史样本切换需部署环境验证。
- 部署迁移后需先成功执行一次容量采集，为已有卷回填稳定身份；在此之前卷性能按约定保持为空。
- 扩大回归时 `test_storage_collection_trigger.py` 的两个既有审计顺序用例因同秒随机 UUID 排序失败，与本次数据关联链路无关，已记录错误日志且未修改无关审计实现。

## 2026-07-18：Mock 超级管理员全页面演示数据

### 已完成

- Mock 网关为超级管理员可见的列表、AI、仪表盘、系统配置和存储分析数据源提供 5 条以上的虚构演示数据。
- Mock Axios 适配器直接返回页面 API 层所需响应，避免额外 `result` 包装导致列表内容为空。

### 验证状态

- RED：新增 mock 数据源契约测试后，`/projects` 仅 2 条记录，按预期失败。
- GREEN：聚焦 Vitest 用例通过；本地 Mock 概览页已显示容量摘要、趋势、项目对比和告警数据。

### 风险与阻塞

- 未逐一进行每个菜单页面的浏览器截图巡检；由数据源契约测试覆盖所有首屏请求。

## 2026-07-18：登录页 Element Plus 图标依赖恢复

### 已完成

- 为 `LoginPage.vue` 直接导入的 `@element-plus/icons-vue` 添加前端生产依赖，避免依赖 pnpm 未提升的间接依赖。
- 同步 `pnpm-lock.yaml`，使其与已声明的测试与运行时依赖保持一致。

### 验证状态

- RED：`pnpm build:prod` 明确无法解析 `@element-plus/icons-vue`。
- GREEN：`pnpm build:prod` 通过，登录页与图标依赖均已进入生产构建产物。

### 风险与阻塞

- 未执行浏览器端登录交互回归；正在运行的 Vite 开发服务需重启后才会使用新的依赖解析结果。

## 2026-07-18：代码审查问题有序修复

### 已完成

- P1 路由测试契约已纳入事件中心页面：在用页面矩阵从 24 更新为 25，懒加载组件总数从 28 更新为 29，并显式 Mock `IncidentCenterPage`。
- P1 Incident 重开逻辑统一按状态判断：同一 30 分钟桶直接命中的已解决事件，也会在新证据到达时重开并写入时间线。
- P1 性能资产映射不再把厂商 UUID/名称绑定到整数 `Volume.id`；只有 ASCII 纯数字标识才增加主键匹配条件。
- P1 r10 降级先移除新终态约束、清空失败行分类错误码，再恢复旧约束；SQLite 使用两阶段表重建保持同一语义。
- P2 AI 对话历史从所属审计恢复未过期的配额确认卡片，只返回确认 ID、剩余有效期和白名单预览；工具参数仍仅保存在 Redis 一次性载荷中。
...
- P3 Mock 快捷账户模板按 Vue 属性换行规则整理，消除会阻塞 CI 的 5 个 `vue/max-attributes-per-line` 错误。

### 验证状态

- RED：页面矩阵与懒加载聚焦测试分别报告“期望 24、实际 25”和“期望 28、实际 29”。
- GREEN：同一聚焦命令通过 `2 files / 12 tests`。
- Incident 同桶重开 RED 用例按预期失败；修复后与既有跨桶 24 小时重开用例合计 `2 passed`。
- 性能资产 UUID 映射 RED 用例确认 SQL 含错误的整数比较；修复后与连续性能异常回归合计 `3 passed`。
- r10 存量失败行降级 RED 复现旧约束冲突；SQLite 实际升降级及 SQLite/PostgreSQL/MySQL 离线迁移合计 `4 passed`。
- AI 历史恢复 RED 用例确认确认卡片缺失；后端 AI/配额聚焦 `48 passed`，前端 AI 页面 `16 passed`。
...
- 全量 ESLint RED 报告登录页 5 个属性换行错误；模板整理后使用同一命令通过且无错误。

### 风险与待完成

- 8 项审查问题、新增的 `pnpm mock` 启动入口和最终 lint 门禁问题均已完成。未执行真实 PostgreSQL/MySQL、Redis/Celery、设备 API 或浏览器交互联调。
- 生产构建保留既有 `%VITE_APP_TITLE%` 和大 chunk 警告。
...
- 未对真实 PostgreSQL 执行 `alembic upgrade head`，以避免在代码合并前改写外部数据库；真实 QuestDB/Redis/Celery、NetApp/PowerScale 回放、邮件/飞书通道和浏览器 E2E 仍需隔离环境验收。
- 本地虚拟环境没有 Ruff，未引入新依赖；静态规则覆盖仍由现有 pytest、迁移编译和 `compileall` 提供。
...
- 工具轮次上限不再直接进入通用服务失败：当前回合禁用工具后总结已有信息，保存 `degraded` 助手消息和审计恢复元数据，页面提供用户明确触发的“继续查询”。
- Provider 返回非 JSON 或非对象工具参数时，仅保留安全的工具名和错误类别；模型最多自动修复两次，耗尽后提供“重新查询”，不展示或持久化原始参数。
- SSE 增加已确认、终态和已知事件载荷校验；协议截断会保留已收到的内容并进入可手动重试失败态。

### 验证状态

- RED：后端 5 个、前端 5 个新增回归场景按预期失败，分别锁定参数修复/降级完成和恢复按钮/SSE 终态缺口。
- GREEN：后端 AI 聚焦测试 `32 passed`；前端 AI 聚焦测试 `24 passed`；目标 ESLint、Python `compileall` 与前端生产构建通过。
- 浏览器：登录态 `http://localhost:5173/ai/chat` 成功加载、会话切换后消息区更新；未见本次新增错误。控制台保留既有 `ICarbonUserFilled` 解析 warning。未发送真实新 AI 查询，降级按钮的真实 Provider 联调由自动化回归覆盖。

### 风险

- 未使用真实 Provider 触发工具上限或连续格式错误；部署后应以脱敏测试会话验证 Provider 对无工具总结提示和恢复回合的实际响应。
...
- 已完成：应用壳改为使用声明式详情面包屑，存储集群详情展示“系统管理 / 存储集群 / 存储集群详情”，其余现有详情入口同步补齐列表父级。
- 验证：`cd frontend && npx vitest run test/unit/pages/storage-cluster-health-analytics.test.js test/unit/chart-coverage-gaps.test.js test/unit/router/routes.test.js test/unit/utils/breadcrumbs.test.js --coverage.enabled=false` 通过，`25 passed`；登录态本地浏览器确认三级面包屑、对象多选项及选择后单行表格结果，图表标题与横轴标签区域分离。
- 风险：对象对比仅作用于当前性能接口返回的 Top-N 集合；若需要跨全部对象筛选、排序或分页，需另行扩展后端查询契约。

## 2026-07-17：AI 助手流式回复与工具轨迹持久化对账

- 目标是让每次 AI 提问在同一条助手回复内并行展示持续 `delta` 文本和工具调用状态，并以 `turn_id`、预创建 `message_id` 与每次调用的 `call_id` 将实时事件、审计和历史消息精确关联。
- 工具开始、完成、失败和取消将保留到对应回复下；可展示 JSON 结果单次最多 `32 KiB`，超出部分标记 `truncated`。会话历史将为助手消息返回回复状态与工具轨迹，旧历史保持兼容。
- 已完成：同一条助手消息从 `accepted` 到终态原位承接实时文本和工具调用；成功、失败和取消均持久化部分文本与轨迹，刷新后按 `turn_id` 恢复。工具结果在写入展示轨迹前脱敏并限制为 `32 KiB`，Provider 历史不会混入预创建的空助手占位消息。
- 验证：后端 AI 测试 `27 passed`；前端聊天、SSE 和 API 聚焦测试与目标 ESLint 通过；`git diff --check` 通过。
- 风险：尚未完成真实登录态浏览器中的长回答、断流、刷新及工具详情联调。聊天输入区固定、内容区高度与滚动布局优化明确留待后续独立任务。
- 专题设计与权限边界见 [AI 助手流式回复与工具轨迹持久化对账](../features/ai-assistant/streaming-tool-trace-reconciliation.md)。

## 2026-07-17：AI 聊天工作台布局

- 已完成：桌面端左侧对话历史保持固定列；右侧由固定页头、独立滚动的消息区和固定发送区组成。发送区在输入框上方展示 AI 风险提示，圆形上箭头发送按钮位于输入框右下角，继续支持 `Ctrl + Enter`。
- 验证：页面聚焦测试 `10 passed`、目标 ESLint 和生产构建通过；浏览器确认 `994px` 视口内无页面纵向溢出，消息滚动区与 `138px` 高发送区边界稳定。
- 风险：真实登录态下的长对话滚动和停止生成仍待与后端流式链路联调。
- 设计见 [AI 聊天工作台布局设计](../superpowers/specs/2026-07-17-ai-chat-layout-design.md)。
...
## 2026-07-18：项目负责人字段展示统一

### 已完成

- 项目列表移除已废弃的“PT经理”列。
- 项目新增、编辑表单移除 `pt_user_id` 选择入口；保留既有 `in_charge_user_id` 字段，并将展示文案统一为“项目负责人”。

### 验证状态

- RED：项目列表响应式列合同测试 `2 failed`，复现旧列名与仍保留 PT 入口。
- GREEN：`pnpm test -- responsive-list-columns.test.js`，`26 passed`；受影响文件 ESLint 通过。
...
## 2026-07-18：容量预测治理页控制区精简

### 已完成

- 移除“容量预测治理”页面标题和副标题，将全局用户可见性开关与“新增候选模型”按钮合并为同一紧凑控制行。
- 保持既有发布开关请求与候选模型创建弹窗行为不变。

### 验证状态

- RED：新增治理页结构契约后，旧标题区仍存在，聚焦 Vitest 按预期失败。
- GREEN：`npx vitest run test/unit/forecast-governance-page.test.js --coverage.enabled=false`，`8 passed`；覆盖紧凑控制行、MAPE 说明、控制行/表格标题尺寸合同与 Grid 顶部紧凑排列，受影响 Vue 文件与测试 ESLint 通过。
- 浏览器核验：`http://localhost:5173/admin/forecast-governance` 桌面端无页面标题/副标题，控制项同一行显示，创建按钮可打开并关闭候选模型弹窗。

### 风险与待验证范围

- 390px 窄屏下全局应用壳仍固定展示侧栏，主内容被裁切；该响应式问题不由本次治理页变更引入，未在本轮扩大处理。

## 2026-07-18：事件中心管理入口与可见性收敛

### 已完成

- 将事件中心从顶级菜单迁移至“系统管理”，路径调整为 `/admin/incidents`。
- 对事件中心子路由显式要求 `superadmin`，避免未来父级菜单权限变更时向普通用户暴露入口。

### 验证状态

- RED：事件中心仍在根菜单，新增路由契约按预期失败。
- GREEN：路由与动态导入聚焦 Vitest `8 passed`，受影响文件 ESLint 通过；已登录超级管理员浏览器中 `/admin/incidents` 正常加载，侧栏仅在“系统管理”子菜单展示事件中心。

## 2026-07-18：表格操作按钮与资源直达链接统一

### 已完成

- 新增共享表格操作按钮组件，固定 `size="small"` 与 `plain`，并按创建、详情、编辑、删除/移除、启用、同步、回滚映射语义颜色。
- 审计并迁移 17 个列表页面或嵌入式表格组件；项目组标签不再使用 `link` 操作按钮，现有“更多”权限和资源能力判断保持不变。
- 新增权限感知的资源链接组件：仅路由 `meta.isAccessible()` 返回 `200` 且所有参数有效时渲染链接；权限不足和缺少 ID 时保留文本。
- 存储层级、项目、项目组和用户目录列表补充已有详情页的直达链接；无详情路由的数据不新增跳转。
- info 控件和选择标签统一为 Slate 灰，图表信息色独立保留青色。
- 资源直达链接继承表格的正常字重，默认使用较浅的品牌蓝，避免比同列文本更粗或更深。

### 验证状态

- RED：新增组件与信息色合同测试在组件、Slate token 和选择标签尚未实现时失败。
- GREEN：`npx vitest run test/unit/components/table-action-button.test.js test/unit/components/accessible-resource-link.test.js test/unit/components/query-form.test.js test/unit/pages/list-action-permissions.test.js test/unit/storage-info-tag-color.test.js`，`66 passed`。
