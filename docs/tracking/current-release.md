...
## 2026-07-17：侧栏图标采用 Logo 同色系

- 仅菜单图标使用 `--sidebar-icon-color`：亮色为 `#76A51D`，深色为 Logo 主绿 `#B0E237`；背景、文字、悬停和选中底色未改变。
- 聚焦样式与路由测试为 `2 files / 6 passed`，目标 ESLint、生产构建和 `git diff --check` 通过；构建保留既有大 chunk 警告。真实登录态浏览器视觉复验未执行。
- 完整设计见 [Logo 同色系导航图标设计](../superpowers/specs/2026-07-17-logo-color-sidebar-design.md)。
