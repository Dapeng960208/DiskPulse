...
- 现象：存储分布的自绘柱状加载动画位置异常，切换后时间筛选栏消失；时间范围内容显示不全，两张主图高度不足。
- 根因：筛选栏位于页签卡片外且显式排除了存储分布；分布加载态使用独立 ECharts 图形并在页签切换期间初始化；主图高度分别硬编码为 `360px` 和 `420px`。
- 修复：筛选栏移入页签内容区并扩宽日期时间字段；存储分布改用 Element Plus 标准加载遮罩；两张主图统一为 `520px`。
- 验证：页面聚焦回归测试通过，`9 passed`；测试覆盖页签内筛选、日期时间文案、加载忙碌状态和图表高度。
- 风险：尚未连接真实后端数据执行浏览器截图复验。
