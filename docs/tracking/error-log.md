...
### 2026-07-20：资源关联事件错误复用容量预测发布开关

- 触发：`capacity_prediction_settings.user_visible=false` 时，项目只读成员打开自己项目内资源的“关联事件”。
- 现象：资源项目权限校验已经通过，接口仍返回 `403 capacity prediction is globally disabled`；跨项目请求也返回 403，但两种拒绝原因被混在一起。
- 根因：`list_resource_related_incidents` 在资源 `reader` 权限校验后又调用 `_require_prediction_visibility`，把预测结果发布策略错误扩展到独立的事件读取能力。
- 修复：关联事件只保留当前资源项目 RBAC 校验；预测详情、最终预测列表、容量计划和预测能力接口继续受全局发布开关约束。
- 验证：回归先确认跨项目仍返回 `403 project permission required`，再确认发布关闭时本项目成员可读取事件；后端修复前 `1 failed`、修复后 `1 passed`，Mock 对应文件 `20 passed`。
- 风险：真实登录态、PostgreSQL 数据和事件规模仍需部署环境验收；自动化仅覆盖服务层项目隔离与发布开关边界。

### 2026-07-20：并发分页旧响应覆盖当前页

- 触发：项目组、用户目录或最终预测列表连续切换分页，较早请求在最新请求之后才成功或失败。
- 现象：旧成功响应会短暂显示错误页数据并提前结束加载；更早失败响应最后完成时还会清空最新结果、把总数归零并显示错误。
- 根因：三个页面的异步查询没有请求身份，只要 Promise 完成就无条件写入共享的 `data`、`total`、`error` 和 `loading`。
- 修复：每个页面维护递增请求序号，成功、失败和 `finally` 只有在序号仍为最新时才能更新状态；最终预测模型版本同时改为候选版本优先、算法版本回退。
- 验证：deferred Promise 回归在修复前共 `4 failed`，分别证明候选版本未展示和旧响应覆盖；修复后最终预测列表 `6 passed`、项目资源页签 `7 passed`，目标 ESLint 通过。
- 风险：机制不取消网络请求，只忽略过期结果；后端仍会完成旧分页查询，但不会污染用户当前看到的状态。

### 2026-07-19：最终预测列表按历史项目快照授权导致越权可见

- 触发：审查 `GET /storage-pulse/api/v1/capacity-predictions` 的项目隔离，并把项目组从项目 1 迁移到项目 2 后，在下一次预测生成前使用项目 1 的只读成员查询列表。
- 现象：原查询只过滤 `capacity_forecasts.project_id` 历史快照，迁移后的项目组及其用户目录仍返回 `total=2`；资源删除后超级管理员和原项目成员也会看到同样的幽灵预测。
- 根因：最终预测列表把预测生成时的归属当作当前授权事实，没有在 `count` 和分页前反查当前 `Group` 或 `StorageUsage → Group` 的项目归属和资源存在性。
- 修复：`list_latest_forecasts` 增加当前资源 `EXISTS` 过滤；项目组按当前 `Group.project_id`，用户目录按当前 `StorageUsage → Group.project_id` 授权，超级管理员保留全项目语义但仍排除已删除资源。异常或超出 32 位范围的历史 `asset_id` 通过受保护的 `CASE` 转为 `NULL`，避免 PostgreSQL 严格整数转换导致整页 500。
- 验证：两条回归在修复前均错误返回 `total=2`；修复后旧项目成员为 0、当前项目成员为 2、超级管理员为 2，删除后所有角色均为 0。异常 `asset_id='abc'/'99999999999'` 的 SQLite 实际查询返回空结果；治理与事件组合 `59 passed`，SQLite/PostgreSQL SQL 编译、`compileall` 和 `git diff --check` 通过。
- 风险：未连接真实 PostgreSQL 执行查询计划或在线数据回放；部署前仍需在隔离环境验证真实数据规模下的执行时间。

### 2026-07-19：导航信息架构 RED 基线与懒加载数量漂移

- 触发：执行导航信息架构需求的 RED 契约测试，覆盖路由、项目详情、容量预测列表和用户目录关联数据。
- 现象：RED 检查点 `99641ce test(frontend): add red navigation architecture contracts` 包含 5 个文件、15 条测试，其中 11 条因项目组/用户目录归属、独立容量预测入口、用户详情关联页签和存储集群二级菜单尚未实现而失败；懒加载合同原先期望 `30`，基线实际统计为 `32`。
- 根因：既有懒加载数量断言未随主线既有路由增长更新；本需求再新增容量预测列表懒加载组件，最终合法数量应为 `33`。
- 修复：将懒加载合同断言修正为 `33`，并在生产代码中实现导航分组、独立容量预测列表和用户目录关联页签；保留既有资源 URL/API。
- 验证：动态导入合同现按 `33` 通过；导航影响面组合 `12 files / 68 passed`，生产构建和 Browser 主流程均已完成。
- 风险：后续新增或移除懒加载路由时仍需同步维护固定数量合同和页面 mock 清单。

### 2026-07-19：懒加载路由合同因测试桩不完整超时

- 触发：在 `frontend` 聚焦执行 `routes-dynamic-import`，将懒加载数量修正为 `33` 后运行全部 loader 的 `Promise.all`。
- 现象：测试未 mock `CapacityPredictionDetailPage`、`AiChatPage`、`AiCenterPage`、`AIAuditDetailPage`、`AuditEventListPage`、`AuditEventDetailPage` 等既有懒加载页，等待 30 秒和 60 秒均未结束。
- 根因：穷举式动态导入合同新增了路由数量，但测试桩集合不完整；超时不是生产路由执行结果。
- 修复：补齐缺失页面 mock，并保留穷举所有懒加载路由的合同；未修改生产路由行为。
- 验证：`routes-dynamic-import.test.js` 已通过，导航影响面组合为 `12 files / 68 passed`。
- 风险：继续增加懒加载路由时，必须同步维护页面 mock 清单，否则合同会再次超时并阻塞验证。

### 2026-07-19：导航聚焦回归暴露页面数量和表格桩缺口

- 触发：执行导航信息架构影响面 Vitest 聚焦组合（19 个文件、150 条测试）。
- 现象：结果为 16 个文件通过、3 个文件失败，146 条通过、4 条失败，另有 2 个未处理异常；内容间距合同仍期望 28 个在用路由页面，当前实际为 29；组件烟测的旧 `ElTableColumn` stub 缺少 `scope.row`，项目组详情链接渲染时产生未处理异常。
- 根因：新增容量预测列表后，页面数量合同未同步；项目详情新增资源链接后，通用表格测试桩没有提供新模板所需的行作用域。
- 修复：将 `CapacityPredictionListPage.vue` 纳入 29 页内容间距合同，为组件烟测补齐 `scope.row`，并把三项存储静态合同的扫描目标从父页面迁移到实际承载表格的 `ProjectGroupsTab.vue`；未扩大断言语义。
- 验证：导航影响面组合 `12 files / 68 passed`，迁移后的三项静态合同 `3 files / 17 passed`，没有未处理异常。
- 风险：新增列表页面或表格插槽时，需同步更新页面合同和共享测试桩，避免出现未处理异常掩盖真实断言结果。

### 2026-07-19：导航发布级全量前端验证保留 11 条既有失败

- 触发：实现收口后执行 `npm test` 和 `npm run test:coverage`。
- 现象：最终全量结果为 `87 files / 557 passed / 11 failed`；失败分布为 `admin-deep-coverage` 4、`page-coverage-gaps` 4、`user-management-ldap-sync` 2、`project-rbac-unified-audit-contract` 1。
- 根因：前 8 条来自既有操作按钮测试桩，LDAP 2 条仍断言已移除入口，AI Chat 1 条缺少 active Pinia；这些失败已在 `main` 基线复现，本任务未修改对应测试文件。
- 修复：本轮只修复由组件迁移直接造成的三条静态合同失败，不扩大到无关测试债务；聚焦、lint、生产构建和浏览器验收继续独立完成。
- 验证：本任务导航影响面 `68 passed`、迁移静态合同 `17 passed`，`npm run lint` 和 `npm run build:prod` 通过；全量与覆盖率仍按非零结果记录，覆盖率未输出最终四项汇总。
- 风险：前端发布门禁仍非全绿；需由独立任务修复 11 条基线测试后重新获得完整覆盖率结果。
