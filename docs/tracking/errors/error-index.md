...
| 错误标题 | 链接 | 出现次数 |
| --- | --- | ---: |
| 前端全量与覆盖率曾保留既有失败 | [错误详情](./frontend/baseline-test-debt.md) | 16 |
| AI 工具遗漏依赖 Query 参数 | [错误详情](./backend/ai-tool-dependency-query-parameters-omitted.md) | 14 |
| Vue 模板属性未按仓库 ESLint 换行规则书写 | [错误详情](./frontend/vue-attribute-line-lint.md) | 4 |
| 独立后端检查在 worktree 缺少默认配置 | [错误详情](./backend/worktree-config-yml-missing.md) | 4 |
| 横切 API 与迁移契约变化后测试预期未同步 | [错误详情](./backend/cross-cutting-contract-test-drift.md) | 3 |
...
| UTC 迁移安全契约尚未落地 | [错误详情](./backend/utc-migration-safety-contract-not-implemented.md) | 2 |
| Claude Code 取消协程未 await | [错误详情](./backend/claude-code-cancellation-coroutine-unawaited.md) | 2 |
...
| PostgreSQL 历史数据修复在 SQLite 结构验证中执行 | [错误详情](./backend/postgresql-data-repair-runs-in-sqlite-structure-test.md) | 2 |
| 已审核厂商事件测试夹具缺少必填处置方案 | [错误详情](./backend/reviewed-vendor-event-fixture-missing-solution.md) | 2 |
| 存储集群健康分析前端传递无时区时间范围 | [错误详情](./frontend/storage-health-analytics-naive-time-range.md) | 1 |
| Router 通用写事务提前结束 SSE 会话 | [错误详情](./backend/router-sse-write-transaction-detached-session.md) | 1 |
| Celery Redis 锁在任务注册期形成循环导入 | [错误详情](./backend/celery-redis-lock-import-cycle.md) | 1 |
| FastAPI 同步 yield 依赖跨线程重置 ContextVar 失败 | [错误详情](./backend/fastapi-yield-dependency-contextvar-cross-thread-reset.md) | 1 |
| AI 用户目录工具混用用户名与整数 ID | [错误详情](./backend/ai-storage-usage-user-id-type-ambiguity.md) | 1 |
| 严格警告模式发现未关闭的 SQLite 连接 | [错误详情](./backend/pytest-unclosed-sqlite-connections.md) | 1 |
| AI 工具展示式参数名触发严格校验失败 | [错误详情](./backend/ai-tool-display-argument-name-rejected.md) | 1 |
| AI 对话工具轮次配置键不匹配 | [错误详情](./backend/ai-chat-tool-iteration-config-key-mismatch.md) | 1 |
| 事件 AI 设置对话框变更未回写页面状态 | [错误详情](./frontend/incident-ai-settings-dialog-state-sync.md) | 1 |
| Incident 关联任务假定会话适配器存在 `info` | [错误详情](./backend/incident-ai-review-session-info.md) | 1 |
| 事件 AI 设置调用已移除 API 方法 | [错误详情](./frontend/incident-ai-settings-api-method-mismatch.md) | 1 |
| 历史事件 AI 研判缺少置信度导致响应序列化失败 | [错误详情](./backend/incident-ai-assessment-confidence-missing.md) | 1 |
