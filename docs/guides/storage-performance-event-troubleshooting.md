...
NetApp/Isilon 已返回 HTTP 错误时，DiskPulse 日志应同时包含设备原始状态码和响应消息；同步管理接口还应保持设备响应的状态码、正文和 `Content-Type`，不能统一显示成 `502`。只有日志中没有设备 HTTP 响应、而是连接、DNS、TLS 或超时时，才按网关/网络问题排查。完整约束见[厂商接口契约](../features/storage-cluster/vendor-api-contracts.md#11-设备-http-错误契约)。
