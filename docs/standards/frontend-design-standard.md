...
- 全站禁止在标题后设置描述性副标题：页面、详情、页签、弹窗和登录页只保留必要标题与操作入口，不得使用“标题 + 说明段落”结构；需要解释复杂术语、风险或操作限制时，使用字段标签、Tooltip、帮助入口或就近提示承载。
...
- 列表或表格的行操作最多两个时直接展示；超过两个时保留一个高频操作，其余收入“更多”，触发按钮计入两个直显操作之一。含行操作的末列必须使用 `fixed="right"` 固定在表格右侧，不得随表格内容横向滚动；同时设置明确的 `width`、`align="right"`、单行展示，并复用全局 `.list-row-actions` 样式；危险菜单项使用 `.list-row-actions__danger`。该规则不处理筛选栏、表头、页面工具栏、卡片操作或弹窗底部按钮。
