...
- 全站列表页和详情页筛选栏新增筛选栏应优先采用 `/runs` 同款紧凑筛选骨架：复用 `analytics-center-filter analytics-center-filter--six`，桌面端固定六列，字段标签使用更小的主题色标题，主操作按钮作为独立 grid 单元固定落在末尾列位（字段少于 4 个时可用 `grid-column: -2/-1` 代替硬码 5/7），不再为单个页面保留另一套筛选布局。存量筛选栏按计划迁移，当前已有布局可维持现状。
