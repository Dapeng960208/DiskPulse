...
- 开始前**必须**阅读本文、`docs/standards/documentation-standard.md`、`docs/standards/domain-terminology.md` 和 `frontend/src/styles/style.scss`。
...
- 默认请求真实后端 API；Mock 仅通过 `VITE_USE_MOCKS=true` 或 `npm run mock` 启用。响应必须保留真实 `data`、`meta`、`traceId` envelope 和字段命名，且不得包含真实用户信息、密钥、token 或敏感地址。
...
- 聚焦验证使用 `cd frontend && npx vitest run <file> --coverage.enabled=false`；全量测试使用 `npm test`；显式覆盖率验证使用 `npm run test:coverage`。Vitest 当前对 Statements、Branches、Functions、Lines 均要求不低于 `80%`。
- Dialog、Select、Popover 等 portal 组件测试必须挂载到 `document.body`，并在 `afterEach` 清理 DOM 和 wrapper。临时 Node 脚本按 ESM 语义编写，不假设 `require()` 可用。
- 提交前运行受影响的 Vitest、必要的 `npm run lint` / 构建检查和 `git diff --check`；最终说明必须如实列出已验证项、未验证范围、原因与风险。
