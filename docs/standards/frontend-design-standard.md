...
- 全站列表页和详情页筛选栏统一复用 `frontend/src/components/form/QueryForm.vue`：字段标签位于控件上方，普通字段宽度保持约 `220–240px`，关键词和时间范围使用 `.query-form-field--wide`；复杂页面把次要条件放入 `advanced` 插槽，并通过 `advancedCount` 和 `active-filters` 展示已启用条件。
