...
- 主筛选和高级筛选共用等宽自适应网格：每行按容器宽度显示 `1–5` 个筛选项，常规最小列宽为 `220px`，容器不足时允许单列继续收缩；所有输入控件占满所在列，`.query-form-field--wide` 不得再改变筛选项宽度。复杂页面通过 `advanced`、`advancedCount` 和 `active-filters` 管理次要条件。
