...
| `502` | NetApp/Isilon 请求失败、异步任务失败或读回不一致。 |

设备凭据继续从 `StorageCluster` 读取，不写入日志、响应或调整记录。服务端日志记录集群 ID、资源类型、资源 ID、设备操作阶段和稳定错误类型，不记录密码、原始认证信息或完整设备响应。
...
现有 `POST /storage-usages/expand` 不作为新页面入口。本期不在设计阶段删除该接口；实现时仅在能够保持旧请求语义和验证覆盖的前提下复用新 service，否则保留并标记为兼容接口，避免未经确认破坏外部调用。
