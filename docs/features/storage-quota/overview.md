...
NetApp 和 Isilon 的 HTTPS 证书校验由 `backend/config.yml` 的 `storage.tls_verify` 控制，默认 `false` 以支持内部自签名设备；设为 `true` 时要求设备证书受 Worker 运行环境信任。存储 API 请求失败会回滚当前集群采集并保留已有资源数据。
