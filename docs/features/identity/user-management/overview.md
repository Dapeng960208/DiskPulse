...
- 用户列表的首次加载和筛选重置默认以 `storage_used` 降序请求，优先展示使用量最高的用户；用户主动点击表头排序后仍按其选择请求。
- 用户存储用量 `storage_used` 以用户目录 `StorageUsage.used` 的 GB 原始值按 `user_id` 汇总；每小时的用户存储统计任务在写入 QuestDB 历史样本时同步回填该字段。没有有效用户目录的用户写入 `0`，避免目录删除后保留历史用量。列表响应继续通过 `capacity.storage_used` 提供显示单位。
...
前端验证入口见[前端测试指南](../../../guides/frontend/testing.md)。真实 LDAP 连通性、字段权限和目录规模仍需在部署环境执行只读快照与同步验收。
