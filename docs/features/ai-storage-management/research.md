...
| AI 助手 | 支持多 Provider、SSE、会话持久化、动态工具、当前用户身份执行、限流、脱敏和 AI 审计；见 [ai_chat_service.py](../../../backend/services/ai_chat_service.py#L382)、[ai_tool_service.py](../../../backend/services/ai_tool_service.py#L122) 和 [AI 后端文档](../ai-chat/backend.md#L19)。 | 已实现 AI 运维助手原型；其工具执行模型不等同于企业级自动化。 |
| 登录与基础安全 | LDAP、JWT、Redis 会话白名单/撤销、超级管理员校验和 AI Key 加密已存在；见 [authentication backend](../authentication/backend.md#L5)、[dependencies.py](../../../backend/dependencies.py#L68)。 | 已实现基础身份认证；仍需企业级权限与秘密管理。 |
...
后续研发已拆分为六份相互依赖、可单独排期和验收的实施计划。应按索引中的依赖图推进：先完成项目级权限与统一审计；模型网关/Secrets 与遥测可观测可在此前置后并行；HA/DR 依赖遥测基线，预测/RCA/事件中心依赖模型网关和遥测基线，受控自治必须最后实施。尤其不得在项目级权限、统一审计、Secrets/KMS、遥测新鲜度和 HA/DR 基线完成前，开放设备写入自治能力。

- [企业级 AI 存储智能运维实施计划索引](../../superpowers/plans/2026-07-17-220000-enterprise-ai-storage-implementation-index.md)
...
- [后端架构](../../overview/backend-architecture.md)
- [存储集群健康分析](../storage-cluster/health-analytics.md)
- [AI 对话后端实现](../ai-chat/backend.md)
- [存储配额软限额与调整](../storage-quota/overview.md)
- [LDAP/JWT 认证](../authentication/backend.md)
