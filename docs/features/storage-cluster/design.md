...
CREATE INDEX idx_storage_usages_cluster ON storage_usages(storage_cluster_id);
```
...
#### 获取所有存储集群
```
GET /storage-pulse/api/storage-clusters
Response: List[StorageClusterResponse]
```

#### 创建存储集群
```
POST /storage-pulse/api/storage-clusters
...
#### 获取集群详情
```
GET /storage-pulse/api/storage-clusters/{id}
Response: StorageClusterResponse
```

#### 更新集群
```
PUT /storage-pulse/api/storage-clusters/{id}
...
#### 删除集群
```
DELETE /storage-pulse/api/storage-clusters/{id}
Response: {"message": "success"}
```

#### 获取集群实时使用趋势
```
GET /storage-pulse/api/storage-clusters/{storage_cluster_id}/realtime
Query Parameters:
...
```
GET /storage-pulse/api/aggregates?storage_cluster_id=1
GET /storage-pulse/api/volumes?storage_cluster_id=1
GET /storage-pulse/api/qtrees?storage_cluster_id=1
GET /storage-pulse/api/storage-usages?storage_cluster_id=1
```
...
def __init__(self, db: Session, logger, storage_cluster_id: int):
    ...
    self.storage_host = cluster.storage_host
    
    # 根据类型初始化客户端
    if self.storage_type == 'netapp':
        self.client = NetAppClient(cluster.storage_host, ...)
    else:
        self.client = IsilonClient(cluster.storage_host, ...)
```
...
1. 前端请求集群列表
   ↓
2. GET /storage-pulse/api/storage-clusters
   ↓
3. 返回所有集群及其使用情况
   ↓
4. 前端选择某个集群
   ↓
5. GET /storage-pulse/api/aggregates?storage_cluster_id=1
   ↓
6. 返回该集群下的所有 Aggregate
   ↓
7. GET /storage-pulse/api/storage-clusters/1/realtime
   ↓
...
# 创建 NetApp 集群
response = requests.post('http://localhost:8000/storage-pulse/api/storage-clusters', json={
...
```python
# 获取所有集群
clusters = requests.get('http://localhost:8000/storage-pulse/api/storage-clusters').json()

# 获取某个集群的详情
cluster = requests.get('http://localhost:8000/storage-pulse/api/storage-clusters/1').json()

# 获取集群的实时使用趋势
realtime = requests.get(
    'http://localhost:8000/storage-pulse/api/storage-clusters/1/realtime',
    params={
...
aggregates = requests.get(
    'http://localhost:8000/storage-pulse/api/aggregates',
