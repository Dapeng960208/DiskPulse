...
页面入口为“管理后台 → 存储集群 → 存储集群详情”。容量、性能和故障分析共用页面时间范围；性能和故障数据在相应页签首次打开时加载。
...
DiskPulse 既有容量告警使用 `source=diskpulse`，严重级别映射为 `high→critical`、`medium→warning`、`low→info`。严重级别统计只接受 `diskpulse`、`netapp`、`isilon` 来源，重复故障只接受 `netapp`、`isilon`；其他来源即使写入 `storage_alerts` 也不进入对应分析。无法唯一归属到存储集群的项目级容量告警保留在原告警范围内，不进入集群健康分析。设备故障指纹由厂商、事件代码、对象类型和对象 ID 组成，不使用可能包含动态内容的完整消息。
...
所有接口要求 `start_time` 和 `end_time`，开始时间必须早于结束时间，范围不得超过 180 天。Top 10 接口支持对象类型和数量参数，数量默认且最多为 10。无数据时返回空集合和可空汇总值，不把无数据表示成故障。
