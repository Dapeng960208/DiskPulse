...
新增 Alembic `000000000007` 迁移。升级删除上述列；降级恢复列及原有默认值。迁移继续兼容 SQLite、PostgreSQL 和 MySQL。
...
- 删除无前端调用的 `/storage-usage/expand` 路由及 `StorageUsageExpand` schema。
...
- `/storage-usage/expand` 从 OpenAPI 中移除；其他用户目录接口不变。
...
4. GREEN 后运行前后端聚焦测试、前端 lint/生产构建、后端 `compileall`、Alembic 单 head 和引用扫描。
5. 浏览器验证系统设置仅显示普通标题、告警规则和保存按钮。

## 8. 风险与边界

- 删除 `/storage-usage/expand` 是公开接口收缩；代码确认当前前端没有调用者，外部脚本若仍调用将收到 `404`。
