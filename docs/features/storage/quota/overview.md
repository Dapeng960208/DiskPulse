...
项目组配额仅由超级管理员调整；用户目录配额由超级管理员或该目录所属项目的项目负责人调整：

- 根导航隐藏项目组和用户目录；项目详情 `/project/:id` 的对应页签按当前项目查看关联资源，既有列表、详情、调整操作和 API 路径不变。
- 用户目录详情 `/usage/:id` 的“配额历史”页签读取 `/storage-usages/{storage_usage_id}/quota/history`；前端仅在 `capabilities.adjust_quota=true` 时请求，服务端继续执行实际授权。项目负责人只能访问其负责项目内用户目录的历史。

- `PATCH /groups/{group_id}/quota` 仅适用于独占存储目标；共享 Volume/Qtree 返回 `409`。
- `PATCH /storage-usages/{storage_usage_id}/quota` 按当前研发用户名调整用户配额；项目负责人不能借此调整其他项目或项目组的配额。
- 硬限额必填；未指定软限额时，服务端和调整表单均默认采用硬限额的 90%，且软限额必须小于硬限额。PowerScale（Isilon）默认采用 7 天软限额宽限期；管理员仍可显式修改。
...
- `POST .../quota/reconcile` 只读设备并修复本地数据；`GET .../quota/history` 返回最近十条脱敏结果。项目组的这两类操作仅限超级管理员，用户目录遵循同一项目负责人边界。
