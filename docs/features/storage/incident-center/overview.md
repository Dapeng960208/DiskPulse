...
- 用户目录详情 `/usage/:id` 另外提供懒加载的“配额历史”“容量预测最终结果”“关联事件”“告警”页签。配额历史仅在资源返回 `capabilities.adjust_quota=true` 时读取；其余页签显示当前用户可见的关联数据，并保留空态和错误态。
...
- 预测页展示容量趋势、P10/P50/P90 区间、耗尽日期、回测 MAPE、日样本量、覆盖率、最新遥测、预测新鲜度、模型版本、来源、容量计划摘要及事件/RCA 边界。最终结果使用 AI 候选时优先显示 `input_quality.candidate_version`，否则回退 `algorithm_version`。无有效数据、遥测过期或 AI 输出失败时必须显示数据质量/回退状态，不得把缺失数据表示为准确率。
- 资源预测响应的总量、容量计划变化量和每个曲线点的 P10/P50/P90 都在 `capacity` 映射中返回 `{ value, unit }`；曲线原始序列以 `data_unit=GB` 标记，前端优先展示字段级单位映射。
