...
- 用户目录详情以懒加载页签展示“容量趋势、配额历史、容量预测最终结果、关联事件”；容量趋势页已提供关联告警表，因此不再提供重复的“告警”页签。容量趋势只展示稳定的目录、限额、已用量和利用率摘要，暂未稳定的扩展字段不作为当前页面契约。
- 配额历史、预测和事件分别遵守配额、预测发布和项目 RBAC 边界；关联告警在容量趋势页遵守资源过滤边界，不重复取数。
