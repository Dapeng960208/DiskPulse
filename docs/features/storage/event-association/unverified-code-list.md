...
`000000000018` 使用《PowerScale OneFS Event Reference Guide》（2021 年 10 月）完整读取 Software events 与 Hardware events 两章，共 499 条事件已取得标题、说明和管理员操作并升级为 `reviewed`。Dell SRS Brevity 表格另有 15 条代码未出现在该版指南中；连同原有 17 条仍缺逐代码版本或处置证据的 PowerScale 代码，以及 10 条 NetApp 代码，本清单当前保留 42 条 `unknown + pending` 定义。
