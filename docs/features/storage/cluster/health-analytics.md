...
下列完整接口路径均要求有效的 `Authorization: Bearer <token>`，并沿用存储集群详情既有的超级管理员访问边界。它们的 JSON 查询子集可作为 AI 工具使用：只有超级管理员能获得工具定义，执行时还会重新校验当前用户；普通用户对话不会看到或调用这些工具。下载型导出不是 JSON 工具，保持不注册。
