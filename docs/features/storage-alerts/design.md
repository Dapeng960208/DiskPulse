...
飞书正文和告警列表使用中文对象、事件类型、限额口径和告警级别。用户目录上下文包含用户名、集群、项目、项目组标签、项目组和 Linux 路径；摘要固定为“集群、项目、Linux目录、事件、使用率”，并从结构化上下文生成，因此历史英文 `description` 无需数据迁移。进入 `failed` 的事件不会自动重放，网络恢复后必须由运维确认收件人与重复风险再处理。
