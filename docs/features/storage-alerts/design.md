...
- 用户目录详情 `/usage/:id` 新增懒加载“告警”页签，复用现有告警查询并固定传入 `related_type=StorageUsage`、`related_id=<usage_id>`，只展示当前目录的关联告警；全局 `/alerts` 列表、告警状态机和服务端项目隔离不变。
...
飞书正文和告警列表使用中文对象、事件类型、限额口径和告警级别。用户目录上下文包含用户名、集群、项目、项目组标签、项目组和 Linux 路径；列表已有独立集群、项目列，因此内容摘要只显示“Linux目录、事件、使用率”，并从结构化上下文生成，历史英文 `description` 无需数据迁移。飞书正文仍保留完整定位信息。进入 `failed` 的事件不会自动重放，网络恢复后必须由运维确认收件人与重复风险再处理。
