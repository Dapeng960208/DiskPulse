...
8. 工具通过内部 ASGI 请求调用原业务 API，并把结果按 Provider 协议追加到上下文；系统管理写请求仅对超级管理员可用。
...
当前标记范围是存储集群、容量池、存储空间、Qtree（NetApp）、项目组标签、用户、AI 模型配置的 CRUD，以及存储设置的读取/更新和离职备份记录的读取/删除。登录、当前用户、LDAP 同步、实时/分析/导出、AI 模型测试、审计查询和回滚操作不注册为管理工具。

执行阶段再次以当前认证用户调用 `is_super_admin`，即使旧注册表被复用也会拒绝普通用户。内部 ASGI 请求使用当前用户 ID 签发的 Bearer Token，继续经过原 API 的认证、权限和数据范围逻辑。写请求的 JSON 内容必须放在受 Pydantic 校验的 `body` 信封内；更新或删除只在用户明确要求时允许模型调用，并先说明影响。
...
- 后端 AI 聚焦测试：`.\.venv\Scripts\python.exe -m pytest backend\test\test_ai_platform.py backend\test\test_ai_services.py -q -p no:cacheprovider`，覆盖轮次上限降级、无工具总结回退、显式空参数、两次参数修复、系统管理注册/二次鉴权、Pydantic `body` 转发和 `204` 删除结果；当前结果为 `39 passed`。
