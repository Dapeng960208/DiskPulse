...
- `audit_events` 追加式模型、查询 CRUD、`GET /storage-pulse/api/v1/audit-events`、`GET /storage-pulse/api/v1/audit-events/{event_id}`、`X-Request-ID`/`X-Trace-ID` 关联中间件和合并后的前向 `000000000009_project_rbac_unified_audit.py` 已存在。
