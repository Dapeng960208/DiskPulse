...
9. 工具调用前还原模型参数，以当前用户身份通过内部 ASGI 请求调用原业务 API；原值工具结果用于 SSE、持久化与审计轨迹，递归混淆后的结果才按 Provider 协议追加到上下文。同一回合内同名同参数的成功结果由服务端复用，不再请求业务接口，轨迹状态为 `reused`，下一次 Provider 调用的工具列表会被置空以强制总结已有结果。标记 `retryable=false` 的失败按工具名阻止本回合再次执行；即使 Provider 在同一批请求中换参数重复调用，也只会收到首次安全失败结果。系统管理写请求仅对超级管理员可用。审计工具同样通过当前用户 Bearer Token 执行，因此模型注册或旧会话不能扩大审计可见范围。
10. 工具参数格式错误会最多反馈给模型两次后重试；工具请求失败时会反馈经脱敏的原因。参数校验错误可据此修正；服务端 5xx 返回 `error_type=server_error` 与 `retryable=false`，明确“不能据此判断参数缺失”，并在下一次 Provider 调用清空工具列表，要求基于已有结果说明数据缺口。只有可重试失败才计入连续三次失败降级；达到工具轮次上限或参数修复耗尽时同样额外发起一次禁用工具的总结调用。
...
常规工具仍以业务路由为唯一参数契约：只有 `GET` 且显式设置 `openapi_extra.ai_exposed=true` 的路由才对登录用户注册。路由自身及其 `Depends` 依赖链中的 Path、Query 参数都会生成 Pydantic 模型，并使用 `extra="forbid"` 拒绝模型擅自增加的参数；可选 Query 参数必须在类型标注中显式包含 `None`，使运行时默认值和 JSON Schema 的 `null` 能力一致。契约测试遍历全部 `ai_exposed` 路由，拒绝缺少类型、互不兼容的多标量类型和“默认 `null` 但 Schema 拒绝 `null`”的字段。执行前会将无歧义的展示式参数名归一到路由字段名，例如 `use ratio min` 和 `use_max` 分别映射为 `use_ratio_min` 和 `use_ratio_max`；无法匹配的 `null` 占位参数会忽略，非空未知参数仍被拒绝。存储集群健康分析的 `start_time`、`end_time` 会明确暴露给模型，任一缺省时在服务端按请求时刻补足最近 24 小时范围。

`list_storage_usages.user_id` 只接受数字 ID 或 `null`；历史客户端传入的空字符串会在边界转换为 `null`，其他字符串返回 `422`。模型按研发用户名查询时使用独立的 `rd_username` 精确匹配参数；`user_id` 与 `rd_username` 不能同时提供。`list_projects.prop` 与 `list_projects.order` 显式声明为可空字符串，并分别说明排序字段和排序方向。
...
当前系统管理工具包括存储集群、容量池、存储空间、项目组标签、用户和 AI 模型配置的非删除已授权操作，以及存储设置的只读查询。项目组与用户目录的限额调整工具仅对超级管理员注册，继续复用原有配额 PATCH 路由、请求校验、设备写入和通知逻辑。大文件、离职备份记录、全部删除工具、创建用户、创建容量池、创建/更新存储空间、Qtree（NetApp）写工具、存储设置更新及 AI 审计查询均不注册。
...
- 审计请求只保存 `[REDACTED]`、长度、用户选择的 `reasoning`、解析后的控制类型和实际发送值；响应保存消息 ID、长度、最终状态及可选恢复操作。Provider 返回的思维链不保存、不展示。工具展示明细和管理写请求的 `body` 按敏感字段脱敏并限长，非法原始参数不保存；工具失败仅展示白名单中的可读原因。服务端 5xx 只展示 HTTP 状态、安全说明、`error_type` 和 `retryable`，不声称参数已通过校验，也不展示异常文本、堆栈或凭据。
...
- 每个对话回合按“工具名 + 规范化参数”缓存成功结果；可重试失败不缓存，允许模型在安全提示下修改参数后重试，连续三次可重试失败才进入受控降级。`retryable=false` 的失败按工具名阻止再次执行，并立即关闭下一轮工具能力。
