...
6. 从当前 FastAPI 应用路由构建工具注册表，并转换为当前 Provider 的工具格式。只要当前用户实际可用的注册表含 `list_audit_events` 或 `get_audit_event`，就在基础或自定义模型提示后追加不可覆盖的审计研判约束：范围检索、事件下钻、`operation_id`/Trace/Request 配对、事实与数据缺口区分，以及四个固定回答标题。
