...
- 审计请求只保存 `[REDACTED]`、长度、用户选择的 `reasoning`、解析后的控制类型和实际发送值；响应保存消息 ID、长度、最终状态及可选恢复操作。Provider 返回的思维链不保存、不展示。工具展示明细和管理写请求的 `body` 按敏感字段脱敏并限长，非法原始参数不保存；工具失败仅展示白名单中的可读原因（如缺少开始/结束时间），其余失败统一回退为通用提示。
