...
工具参数由路由 Path/Query 参数和写请求的 Pydantic `body` 信封动态校验。全部 `ai_exposed` 路由必须提供单一、无歧义的 JSON Schema 类型，并让可空默认值与 Schema 一致；自动契约测试会扫描所有工具字段。`list_storage_usages` 的 `user_id` 只接受数字 ID 或 `null`，按研发用户名查询使用独立的 `rd_username`，二者不能同时提供。执行阶段再次校验超级管理员身份，再携带当前用户 Bearer Token 通过内部 ASGI 调用原业务 API；因此旧注册表不能绕过权限。转发请求体时不包含 Pydantic 计算字段；删除接口的 `204 No Content` 视为成功空结果。
