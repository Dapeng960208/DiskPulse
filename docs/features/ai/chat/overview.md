...
工具轮次达到 `max_tool_iterations` 后不会直接把已有数据丢弃为服务故障：当前回合禁止再调用工具并总结已获得信息，助手消息标记为 `degraded`，附带“继续查询”操作。工具参数不是 JSON 对象时，系统最多两次要求模型按工具契约修复；工具请求失败时也会安全提示模型修改参数后重试，连续三次失败则降级为基于已有结果的回答并附带“重新查询”操作。原始非法参数不会展示或持久化。相同工具与相同参数在同一回合已成功时，服务端复用该结果而不再次请求业务接口，工具轨迹标为“复用已获取结果”。只有用户点击恢复操作才会开始新的受限查询回合。
