...
配额调整工具会先返回待确认卡片；用户确认后，卡片必须基于确认接口的实际结果显示“配额调整成功”或可读失败原因，不得只结束等待状态。配额值的默认规则见[配额](../../storage/quota/overview.md)。
...
常规分析工具包括当前用户可见的性能异常列表、故障分析事件列表和 Incident 诊断摘要，模型可先定位事件再读取确定性诊断。存储空间性能监控仅向超级管理员注册 `GET /volumes/{volume_id}/monitoring/ai`；它只返回性能指标、容量趋势和项目绑定摘要，不返回目录路径。
