...
| `src/pages/admin/ai/AiCenterPage.vue` | 超级管理员模型配置、默认模型与名称混淆设置、连接测试和审计分页筛选。 |
...
- AI 中心提供默认开启的“AI 名称混淆”开关。关闭前必须明确确认“后续 AI 对话会向模型发送真实资源名称”；开关保存期间禁用交互并显示保存状态，失败时恢复上次值并显示错误。它只提交 `name_obfuscation_enabled`，不覆盖默认模型设置。
