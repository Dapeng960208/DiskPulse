...
`_performance_findings` 保留异常观察的三连续 5 分钟桶和鲁棒 Z 分数规则。只有延迟序列连续三点均正向、每点 Z 不低于 `3.5`、每点至少有 12 个季节性样本，且每点延迟增加达到 `max(5 ms, 基线 × 50%)` 时，才把 `incident_eligible=true` 传给 Incident 关联；IOPS 和吞吐量永远只保留为异常观察和受限上下文。对 IOPS 且最近三点 MAD 均为零的序列，只有三点都不超过动态门槛才抑制；资源至少需要 12 个近 28 天样本，否则回退同集群至少 12 个样本的中位数，仍不足时只用绝对下限。抑制发生在 `AnomalyObservation` 写入之前。
