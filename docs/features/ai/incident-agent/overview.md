...
研判输入由服务端按事件选择受限事实，模型不能自行指定查询对象或时间范围。性能观测使用当前事件资产在最近证据时间前 24 小时内的读/写/总延迟、IOPS 和吞吐小时聚合，以及采样、缺失小时和指标汇总；无样本、资产映射不全或查询不可用会显式标为数据缺口。输入不包含检测算法的事件类别、确定性严重程度、诊断候选/分数/置信度、MAD 等鲁棒统计或旧的异常触发摘要，避免先验结论污染研判。
...
IOPS 降噪属于确定性检测，始终生效而不依赖 Agent。IOPS 与吞吐量偏离保留为 Agent 的只读诊断上下文，但不能单独创建性能争用 Incident；性能建单仅由持续、正向且达到绝对影响门槛的 P95 延迟退化决定。其详细算法和事件中心行为见[事件中心专题](../../storage/incident-center/overview.md)。
