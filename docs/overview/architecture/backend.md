...
系统跨层关系见[系统架构](./system.md)，存储职责与迁移规则见[数据库架构](./database.md)。具体业务行为以 `docs/features/<领域>/<功能>/` 中的专题文档为准。
...
| `redis` | Celery broker/backend、认证会话、AI 限流和任务锁 |
...
- PostgreSQL 迁移由 Alembic 管理，QuestDB 使用独立的前向迁移与 checksum 账本。部署以当前迁移目录为准，不得手工伪造版本或依赖历史 revision 编号。
