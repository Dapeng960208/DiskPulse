...
模型、迁移、索引、查询和跨方言兼容规则以[数据库开发规范](../../standards/database/database-development-standard.md)为准；所有 PostgreSQL 与 QuestDB 瞬时字段还必须遵守[UTC 时间契约](./time-contract.md)。后端请求生命周期、服务边界和任务入口见[后端架构](./backend.md)。
