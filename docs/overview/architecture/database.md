...
PostgreSQL 模型变更由 Alembic 前向迁移管理。QuestDB 使用独立 SQL 前向迁移，`backend/questdb/migrate.py` 按版本和 checksum 执行并记录已应用版本。Redis 不是业务事实的长期存储，依赖 Redis 的授权或确认流程在 Redis 不可用时必须安全失败。
