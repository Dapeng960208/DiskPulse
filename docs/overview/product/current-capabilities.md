...
| 能力域 | 当前行为 | 事实来源 |
| --- | --- | --- |
| 存储采集与资源映射 | 统一展示 NetApp 与 PowerScale 的存储集群、容量池、存储空间、Qtree（NetApp）和用户目录。 | [存储集群总览](../../features/storage/cluster/overview.md) |
...
| 项目隔离与审计 | 项目成员角色、项目数据隔离、能力字段和追加式统一审计。 | [RBAC 与审计](../../features/identity/project-rbac/backend.md) |
...
| 预测与事件中心 | 容量预测、异常/RCA、事件处置和项目范围访问控制。 | [预测、RCA 与事件中心](../../features/storage/incident-center/overview.md) |
