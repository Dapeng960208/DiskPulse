...
| 预测与事件中心 | 一级容量预测列表、资源最终预测、异常/RCA、仅紧急集群事件进入的处置队列，以及项目范围访问控制。内部 `device_fault` 类别展示为“设备健康风险”；未分类严重事件可保守准入，但不宣称已确认故障。 | [预测、RCA 与事件中心](../../features/storage/incident-center/overview.md) |
