...
- 首页改为近 30 天图表化存储总览，集中展示容量摘要、容量趋势、使用率、容量对比和告警趋势；选择项目后增加项目内用户使用容量 Top 10。
- 默认汇总启用存储集群的物理容量，并按项目对比配额；项目视图原位切换为项目限额、项目组容量对比、用户排行和项目相关告警。
- Dashboard 按摘要、容量趋势、容量对比、告警趋势和用户排行拆分为 5 个认证接口；单个数据块失败或加载时不会遮挡整页，QuestDB 不可用时只降级容量趋势。
