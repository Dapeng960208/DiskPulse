...
## 2026-07-16：存储告警规则进入设计阶段（待实现）

- 已形成系统、项目、项目组三级完整规则及继承契约，覆盖硬/软限额、连续两次确认、升级、静默降级、重复和恢复状态机。
- 已明确采集事务提交后异步评估、飞书收件人和 CC/debug 矩阵、1/5/15 分钟重试、迁移与后台页面方案。
- 当前仅完成设计，接口、迁移、任务和页面尚未实现；开工时发现的 1 个后端迁移测试定位问题和 2 个前端旧术语断言已在 `d16e8f1` 修复，复验为后端 `284 passed`、前端 39 个测试文件共 `196 passed`。
