...
## 2026-07-16：项目组与用户目录操作列优化

- 项目组和用户目录列表统一为“详情 + 更多”：详情对所有用户直接可见，管理员专属操作收进更多菜单，操作列保持单行对齐，不再随按钮数量不规则换行。
- 项目组管理员菜单提供调整配额、编辑和删除；用户目录管理员菜单只提供当前已有的调整配额。添加项目组和新增用户目录入口统一只对管理员显示。
- 权限继续复用 `disk-monitor:admin`，并兼容现有 `superadmin` 与 `*:root`；未新增前端依赖、后端接口或角色类型。完整边界见 [列表操作列样式与权限设计](../features/storage-quota/list-action-design.md)。
