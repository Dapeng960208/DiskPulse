...
- 首页改为近 30 天图表化存储总览，集中展示容量摘要、容量趋势、使用率、容量对比和告警趋势；说明文字保持最少。
- 默认汇总启用存储集群的物理容量，并按项目对比配额；选择项目后原位切换为项目限额、项目组容量对比和项目相关告警。
- 新增认证聚合接口 `/storage-pulse/api/dashboard/overview`；QuestDB 不可用时只降级容量趋势，容量快照和告警仍可查看。
